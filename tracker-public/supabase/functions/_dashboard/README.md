...
2. **Token-level equality.** Both sides are stripped of comments, imports,
   ...
   The build fails on any divergence and prints where. Current: **7648 tokens**
...
4. **Parsed.** Both bundles parse clean under the TypeScript compiler's own
   parser, as do all five sources. Verified against a deliberately broken
   control, because a parser that silently accepts everything proves nothing.

## Known gap

The 98-test suite in `_test/` needs Deno, which was not available where these
